...
- **Primary data source: Apify `memo23/streeteasy-ppr`** — Pass 1 INTERMITTENT (proxy IP rotation; cron's daily 09:00 UTC slot sometimes hits blocked IPs). Pass 2 for sales FIXED (Session 19): memo23 patched `/sale/{id}` path to pull financials from non-PX-blocked source. Pass 2 for rentals FIXED (2026-05-10): memo23 shipped same bypass for `/rental/{id}` — `normalize_rental()` now handles `combineData_rental_*` (original), `rentalCombineResponse_rental_*` (bypass, mirrors sale `_SC`), and bare `rental_*` (fallback). 4 pass1-only rentals will upgrade on next cron run. **VALIDATION PENDING:** new namespace names inferred by analogy from sale schema, not confirmed from real actor output. Verify listings 5022439, 5022432, 5020246, 5025162 reach pass2 after 2026-05-11 09:00 UTC cron. If not, dump raw response on 5022439 and update namespace constants. Actor's new build uses different field schema (`pricing_*`, `propertyDetails_*`, `saleCombineResponse_sale_*`) — `normalize()` in pull.py handles both old and new schemas.
...
- **W7 rental sentinel fix (2026-05-10)** — `verify_stale_shortlists()` previously checked for off-market status using only `price`, `pricing_price`, `saleCombineResponse_sale_price` — none of which exist in rental Pass 2 responses (rental price is nested in `price_histories_json`). Fixed: for rental entries, also checks `combineData_rental_price_histories_json`, `rentalCombineResponse_rental_price_histories_json`, and `rental_price_histories_json`. Without this fix, any shortlisted rental would have been incorrectly confirmed off-market the first time W7 ran against it.
