...
- **db.json state (Session 37, 2026-05-19):** 416 active listings (340 sale, 75 rental, 1 delisted). **416 at pass2 quality, 0 pass1, 0 partial.** Cron running cleanly daily. 24 misclassified listings manually patched across Sessions 36–37 (see normalize() misclassification note below).
- **normalize() misclassification (ongoing):** The actor sometimes returns `"condo"` for listings that are actually co-ops, condops, or houses. **Session 36 fix:** the condo path in `normalize()` now falls back to `old_maint` when `maint_fee` is None, preserving maintenance fees for misclassified co-ops/condops. **15 listings patched in Session 36:** 6 co-ops and 3 condops got `type→coop` + `maintenance` populated from StreetEasy; 6 townhouses/houses got `type→house` + `monthly_taxes` populated. **Session 37 gap discovered:** the Session 36 fix preserved the `maintenance` field but did NOT correct the `type` field — so misclassified listings had maintenance in the DB but `calcMonthlyTotal` still used the condo path (zeroing fees). **9 more listings patched in Session 37** (1 user-reported + 8 from a full-DB sweep): fingerprint is `type=condo` + `maintenance>0` + `monthly_fees=null`. **Sweep query to catch future regressions:** `[v for v in listings.values() if v.get('type')=='condo' and v.get('maintenance') and not v.get('monthly_fees')]`. Houses have no common charges — `calcMonthlyTotal`'s condo path handles them correctly (`monthly_fees||0 + monthly_taxes||0`). Townhouse taxes are not returned by the actor at all; must be patched manually. **Open follow-up:** add auto-correction to normalize() that sets `type='coop'` when the fingerprint matches, so future ingestions self-heal.
