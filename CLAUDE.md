...
- `.github/workflows/refresh.yml` — cron 1×/week (Sunday 00:00 UTC ≈ Saturday 8pm ET; cut from 2×/day in Session 49, 2026-08-07 — Omar closed on his apartment, so StreetHard is now maintenance-only, not an active hunt tool. Was cut from 4×/day to 2×/day in Session 47, commit `9a86d24`); calls Apify (Pass 1/2 + capped stale-refresh), commits data/, Pages auto-deploys
- Apify token in `.env` locally; `APIFY_TOKEN` GitHub Secret in CI
- **Apify usage cap (Session 47):** the account has a custom monthly platform-usage limit. When hit, Apify returns **403 Forbidden on actor start** and every cron run fails in ~13s ("Pass 1 failed — Failed to start Apify run: 403"). Fix: raise the limit in Apify Console → Billing (Omar did this Jul 2026 after burning $50 in 11 days at 4×/day, ~$4.50/day). At 1×/week (Session 49) expect a small fraction of that — roughly one run's cost per week instead of per day.
- **W5 cliff guard is unaffected by the weekly cadence** — `check_pass1_coverage()` baselines off the last `COVERAGE_WINDOW=7` *healthy runs* in `pipeline_health.json` (a rolling run count, not a calendar-day window), so it still works correctly at 1×/week; the baseline just now spans ~7 weeks of wall-clock time instead of 7 days. Code comments calling it "rolling-7-day" are a stale label from the daily-cadence era, not a bug.
...
- **Default sort**: Per tab — Inbox: Days Listed asc (most recently listed first, changed from Monthly Pmt desc in Session 49), Shortlist: OQ# asc, Archive: bucket_changed_at desc. **Sticky as of Session 48** — these are first-visit-only fallbacks now; once a bucket has been manually sorted, that choice persists (see Sort control bullet below).
...
- **Mobile text-input font size (Session 48 fix)**: every `input[type="text"]`, `input[type="number"]`, and `textarea` is forced to `font-size: 16px !important` inside `@media (max-width:768px)`. Below 16px, iOS/Android auto-zoom the page on focus and don't reverse it on blur — this was leaving the app stuck zoomed in after every note edit (also hit rank inputs, search, and the mortgage bar fields). `!important` is required: several of these fields carry component-specific font-size rules with higher specificity than a plain element selector. Any new text-entry field added later inherits this automatically — no per-component fix needed.
- **Sort control (Session 48)**: a "Sort" button sits next to "Filters" in the filter bar — desktop popover / mobile bottom sheet, same `.filter-popover` CSS as Filters (new `.sort-wrap`/`.sort-popover` markup, no duplicated styling). Lists every sortable field the table headers already support (Monthly Pmt, Fees/Mo, % Fees, Ask Price, SqFt, Beds/Baths, Price/SqFt, PMT/SqFt, Type, Yr Built, Days Listed, Building/Unit), plus OQ #/RQ # gated to the Shortlist tab (`.sort-popover.hide-ranks .sort-row-rank`, toggled alongside the table's existing `.hide-ranks`). Tapping a field calls the same `sortBy(col)` the `<th>` headers use — tap again to flip asc/desc. `updateSortMenuUI()` keeps the menu's active-row highlight/arrow and the Sort button's own arrow in sync with `sortCol`/`sortDir`; called from `sortBy()`, `setBucket()`, and initial load. Opening Sort closes Filters and vice versa; both share `#filter-backdrop` on mobile. This exists because table view could already toggle asc/desc by tapping a column header twice, but Card view — mobile's default — had no sort control at all.
- **Sticky sort (Session 48)**: `sortBy()` writes the chosen `{col, dir}` to `localStorage` under `sh_sort_prefs`, keyed per bucket. `bucketSortDefault(bucket)` (declared alongside `sortCol`/`sortDir` near the top of the script) reads it back in `setBucket()` and on initial load, falling back to the original T6 hardcoded defaults for any bucket never manually sorted. Deliberately per-bucket, not global — Shortlist keeps its OQ#-rank default meaningful on a fresh visit even if Inbox was last sorted by something else. `localStorage` calls are try/caught (private-browsing can throw); worst case sort just isn't sticky, nothing else breaks.
...
- `BUILDINGS-FEATURE-PLAN.md` — spec + 3-hat (CPO/CTO/Architect) review for the Buildings tab, shared target list, and targeted-building highlight (Session 44). Records the locked decisions A–E and SHIPPED status.
...
- `scripts/stale_refresh.py` — stale-listing Pass 2 refresh sweep (Session 43). Re-prices listings that aged out of the Pass 1 search sample and runs W9 off-market detection. Phased `--submit`/`--collect` for the sandbox (Cowork kills detached processes); `--run [--cap N]` end-to-end elsewhere. The cron calls `--run --cap 60` after each pull (1×/week since Session 49; was 2×/day Sessions 47–49) to keep prices + off-market flags current across the whole Inbox.
- `scripts/backfill_names.py` — one-off `building_name` backfill (Session 44). Fetches the actor `slug` for existing listings and derives proper building names via `pull.deslugify_building_name`. Phased `--submit [N]`/`--collect`/`--status` for the sandbox; state in `/tmp/bf_run.json` + `/tmp/bf_processed.json`. Re-run after a normalizer change by removing the processed file.
