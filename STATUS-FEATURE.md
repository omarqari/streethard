...
**Removed in Session 26.** Write endpoints are now public — no API key required. CORS restricts browser-based writes to `streethard.omarqari.com`. This means any family member can edit notes and rankings without any per-device setup.

Previously used a `WRITE_API_KEY` in localStorage + `X-API-Key` header, but this created friction (each browser needed the key pasted via a Settings panel). Since the site URL is not publicly advertised and the data isn't sensitive, open writes are appropriate.

### CORS

Allow exactly `https://streethard.omarqari.com`. `Access-Control-Allow-Headers: Content-Type`. Methods: `GET, PUT, POST, OPTIONS`.
