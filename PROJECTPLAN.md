...
### State Model (Revised Session 21 — Three-Bucket Triage)

Three buckets: `inbox` (default, new listings land here), `shortlist` (actively pursuing, has OQ/RQ rankings), `archive` (rejected/passed). Replaces the old six-status enum + watch boolean. Transitions are explicit button clicks. Auto-resurrection: archived listings whose price drops below `price_at_archive` auto-promote to Inbox on page load. OQ/RQ rankings are Shortlist-exclusive and server-cleared on exit. Plus free-text `notes` (persist across all buckets) and `chips` (Shortlist only in v1). Full schema in STATUS-FEATURE.md.
...
- **v1** — ship it. Three-bucket triage (Inbox/Shortlist/Archive), tab navigation with URL hash, transition buttons, auto-resurrection on price drops, OQ/RQ rankings (Shortlist-only, server-enforced clearing), notes, Settings panel, optimistic UI, offline outbox. Acceptance criteria in TASKS.md.
- **v1.1** — triage speed. Bulk archive (checkboxes + toolbar). Keyboard shortcut `e`. Chips on Shortlist items.
- **v1.5** — niceties. Service Worker + IndexedDB for closed-tab offline. Per-IP rate limit. Export-to-CSV of shortlisted listings.
- **v2** — only if needed. Per-user identity, multi-key with attribution. Status history / audit trail. Push notifications. None required for n=1.
...
### Still-Open Questions (Session 21)

- **DNS cutover timing.** Drop the fallback CORS origin once both devices verify load via `streethard.omarqari.com`.
- **Mobile Safari `localStorage` eviction.** iOS 17 wipes site data after ~7 days of non-use. Accept periodic re-paste at v1; mitigate in v1.5 if annoying.
- **Chip vocabulary.** `no light`, `bad layout`, `building risk`, `priced too high`, `noise`, `condition`, `bad block`, `flip tax`, `board risk`. Confirm before T10 (chips on Shortlist) ships.
...
Run the 15-minute due diligence checklist per serious candidate. The three-bucket triage system (Inbox → Shortlist → Archive) is the in-app tool for this phase. Shortlisted items get OQ/RQ rankings, notes, and chips for due diligence tracking. Backend is Railway-hosted FastAPI + Postgres (decided Session 15, built Session 18). See STATUS-FEATURE.md for the full design.
