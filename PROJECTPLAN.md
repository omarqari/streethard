...
### Open Questions (must resolve before code is written)

- **Final status names.** The proposed set is `watching / viewing / shortlisted / rejected / offered` plus implicit `none`. Confirm or rename. Once persisted, renaming requires an idempotent CHECK-constraint swap and a client update — straightforward but worth doing once.
- **Backups posture.** Confirm "Railway snapshots are sufficient for v1" or surface an actual concern.
- **Hobby tier signup.** Confirm willingness to pay $5/mo so the service doesn't sleep. Pre-Hobby first-click latency is a noticeable annoyance, not just a benchmark number.
